...
| `manage_projects` | `POST /api/v1/projects`, MCP `create_project` |
...
OAuth scopes use the same strings (space-separated in the OAuth protocol). Dashboard create UI exposes the first four as checkboxes.
