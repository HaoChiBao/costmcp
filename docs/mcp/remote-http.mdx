...
| Method | Result |
|--------|--------|
| `initialize` | Protocol + `serverInfo: { name: "costmcp", version: "0.2.0" }` |
| `notifications/initialized` | `202` empty |
| `notifications/cancelled` | `202` empty |
| `ping` | `{}` |
| `tools/list` | Full tool catalog (usage, expenses, subscriptions, obligations, projects, estimates, summaries) |
